...
public class SmartParkingDriverRaspi implements BundleActivator {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		GpioPinListenerDigital listener = new GpioPinListenerDigital() {
			...
			public void handleGpioPinDigitalStateChangeEvent(
				...
				System.out.println(" --> GPIO PIN STATE CHANGE: "
						+ event.getPin().getPin() + " = " + event.getState());
				GpioPinDigitalOutput gpioPinDigitalOutput = ledMap.get(event
						.getPin().getPin());
				...
				try {
					RestClient.callRestWebService(JSONConverter
							.convertToJson(parkingSensorData));
				} catch (Exception e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
				System.out.println("Total number of available parking = "
						+ total_num_available_parking);
			}
		};
		...
		ledMap.put(RaspiPin.GPIO_00, gpio.provisionDigitalOutputPin(
				RaspiPin.GPIO_04, "Parking_1", PinState.HIGH));
		ledMap.put(RaspiPin.GPIO_01, gpio.provisionDigitalOutputPin(
				RaspiPin.GPIO_05, "Parking_2", PinState.HIGH));
		ledMap.put(RaspiPin.GPIO_02, gpio.provisionDigitalOutputPin(
				RaspiPin.GPIO_06, "Parking_3", PinState.HIGH));
		ledMap.put(RaspiPin.GPIO_03, gpio.provisionDigitalOutputPin(
				RaspiPin.GPIO_07, "Parking_4", PinState.HIGH));
